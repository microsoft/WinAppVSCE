## Description

<!-- Briefly describe what this PR does and why -->

## Usage Example

<!-- If this PR adds or changes extension commands, launch.json fields, manifest editor behavior, or packaging flows, include a short example -->
<!-- Example:
```jsonc
{
  "type": "winapp",
  "request": "launch",
  "name": "WinApp: Launch and Attach"
}
```
-->

## Related Issue

<!-- Link to any related issues: Fixes #123, Closes #456, Related to #789 -->

## Type of Change

<!-- Keep the applicable line(s), delete the rest -->

- 🐛 Bug fix
- ✨ New feature
- 💥 Breaking change
- 📝 Documentation
- 🔧 Config/build
- ♻️ Refactoring
- 🧪 Test update

## Checklist
<!-- Delete the ones that do not apply to your changes -->

- [ ] New tests added for new functionality (if applicable)
- [ ] Ran `.\scripts\build-vsce.ps1` locally
- [ ] Tested locally on Windows (if applicable)
- [ ] [README.md](../README.md) updated (if applicable)
- [ ] [CONTRIBUTING.md](../CONTRIBUTING.md) updated (if contributor workflow changed)
- [ ] `package.json` contribution points / command/debugger/custom-editor docs updated (if extension surface changed)
- [ ] Screenshots or GIFs added for UI changes (if applicable)

## Screenshots / Demo

<!-- If applicable, add screenshots or GIFs demonstrating the changes -->

## Additional Notes

<!-- Any additional information that reviewers should know -->

## AI Description

<!-- ai-description-start -->
_This section is auto-generated by AI when the PR is opened or updated. To opt out, delete this entire section including the marker comments._
<!-- ai-description-end -->
