...
async function codeActionsAtOnly(buffer, matchCode, matchText, onlyKind) {
  ...
  // Match the diagnostic by its flagged SPAN TEXT (not just the message) so a stale diagnostic from a
  // previous probe (same code + message) can't be picked up — the same race-guard h.codeActionsAt uses.
