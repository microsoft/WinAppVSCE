...
describe("WinUI XAML red-team 70 (method hover returns/params adversarial)", function () {
  ...
  // Promoted from independent first-hand triage (paths the initial sweep did not lock):
  ...
  it("red-team 70 event-handler VALUE hover (undocumented method) is byte-identical fence-only", async () => {
    // Distinct resolution path from the {x:Bind ...()} function binding: the handler ATTRIBUTE VALUE
    // resolves to the OnGo_Click method directly. Undocumented => must stay fence-only (no phantom sections).
