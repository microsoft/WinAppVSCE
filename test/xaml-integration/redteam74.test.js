...
// Round 74 adversarial probes: resource-key completion must be type-scoped without ever
// false-hiding valid framework keys or conservative author keys.
...
describe("WinUI XAML red-team 74 — type-scoped resource key completion", function () {
  ...
  it("red-team 74 unrelated GridLength targets hide typed framework keys and type-incompatible author keys but keep the App key", async () => {
    ...
    for (const [name, inner] of probes) {
      ...
      // Round 78: the doc-local Brush author key is not assignable to GridLength -> hidden.
  ...
  it("red-team 74 document-local author keys are type-scoped by declaring element type; App keys stay conservative (round 78)", async () => {
    // The two doc-local author keys are both <SolidColorBrush> (Brush) -> offered on a Brush
    // target, hidden on incompatible Style/Color/CornerRadius targets. The App.xaml key is
    // NEVER type-scoped (arrives as a bare string with no declaring element).
    const probes = [
