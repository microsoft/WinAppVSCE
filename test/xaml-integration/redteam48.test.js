...
// Round 48 red-team probes for WXAML0001 "Add xmlns:…" quick fixes.
// These intentionally avoid h.NS for most probes because h.NS already declares x, d, and mc.
...
describe("WinUI XAML — red-team 48 (Add xmlns quick fix)", function () {
  ...
  it("still offers a correct Add xmlns fix on the valid root when a trailing child is unterminated", async () => {
    // A valid root <Page …> with an unterminated trailing child (<d:Foo with no '>' or '/>') is a
    // MID-EDIT state, NOT a rootless document: the root tag is well-formed and the prefix `d` is
    // genuinely undeclared, so offering to declare xmlns:d on the root is correct — it is exactly
    // what Visual Studio does while you type. The separate unterminated-tag diagnostic (XAML0007) is
    // unrelated and does not make the namespace fix bogus. Only a truly rootless doc (root?.Name is
    // null — see the empty/comment-only test above) suppresses the action. Verified on the real
    // server: each buffer emits WXAML0001 on "d" and the applied edit declares xmlns:d on the root.
    for (const buffer of [
