...
// WXAML0005 diagnostics for non-first x:Bind path segments.
...
describe("WinUI XAML red-team 17 — x:Bind non-first diagnostics false-positive hunt", function () {
  ...
  it.skip("known accepted edge: object-only member after interface-return method", async () => {
    // KNOWN-ACCEPTED-EDGE: the shared bindable-member oracle intentionally excludes System.Object-declared members on interface return types.
  ...
  it("does not flag valid function-binding argument member chains", async () => {
    // Valid function-binding argument chains remain diagnostic-free.
    const buffer = page([
      "<StackPanel>",
  ...
  it("validates member chains after a cast against the cast target type (round 33)", async () => {
    // Casts rebind the x:Bind root before validating the remaining member chain.
