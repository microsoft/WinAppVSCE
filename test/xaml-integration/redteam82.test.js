...
describe("WinUI XAML — red-team 82 TemplateBinding member navigation", function () {
  ...
  // Boundary-inclusive by design, identical to the shipped {x:Static}/{x:Type} resolvers (same
  // ValueSpan.ContainsInclusive hit-test): a caret at the FIRST character of the member — even when the
  // extension name is followed by extra spaces — is "on" the member and resolves it. The value span excludes
  // leading whitespace (the parser skips it before the arg), so only the member-start edge is inclusive;
  // genuine interior whitespace BEFORE the member start stays silent (proven by the "space after name" probe).
  ...
  // The member's RIGHT edge is inclusive too (a caret immediately after the last character, before "}",
  // resolves — the universal IDE convention, matching {x:Static}). But a caret PAST a trailing space, beyond
  // the trimmed value span, is silent — proving the span boundary is exact, not greedy.
  ...
  // Tolerant parsing keeps IntelliSense alive MID-TYPE: an unterminated extension (no closing brace yet — as
  // while the user is still typing) still resolves the member the caret is on, identical to the shipped
  // {x:Static}/x:Bind resolvers. Crash-safety for empty/malformed forms is covered above; F12 stays [] because
  // the member is framework metadata.
