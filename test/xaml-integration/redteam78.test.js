...
// Round 78 adversarial probes: document-local author keys must be type-scoped by
// their declaring element type without false-hiding compatible author resources.
...
describe("WinUI XAML red-team 78 — document-local author-key type-scoping", function () {
  ...
  it("red-team 78 value-type Setter.Property (FontSize double) offers x:Double and hides Brush/Style", async () => {
    // The Brush Setter path is covered above; this pins the VALUE-TYPE Setter.Property branch so a
    // compatible x:Double is never false-hidden when the target resolves through Setter.Property.
    const buffer = withResources(`<Style TargetType="TextBlock">
  ...
  it("red-team 78 control-instance key is offered on a base-typed property and hidden on a Brush property", async () => {
    // A <Button> resource used on Border.Child (typed UIElement) must be offered via real control-hierarchy
    // bidirectional assignability — the worst outcome would be false-hiding a valid control resource.
