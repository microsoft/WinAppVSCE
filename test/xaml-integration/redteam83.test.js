...
describe("WinUI XAML — red-team 83 parenthesized Storyboard.TargetProperty member navigation", function () {
  ...
  // Round-83 inclusive end-of-token convention: a caret touching the trailing ')' resolves the member it
  // abuts — identical to the shipped round-81 sibling (ResolveVsmTargetMemberAsync excludes only offset >
  // memberEnd, so the exclusive-end position IS on the member). A caret AFTER the ')' is silent (see the
  // between-groups / open-paren cases), so the inside/outside asymmetry is correct — NOT a defect.
  ...
  // Round-83 tolerant mid-type parse: an unterminated "(Owner.Member" group (no ')') resolves the member as
  // the user types — consistent with the round-77 completion counterpart, which lists members for the
  // unterminated "(Canvas." group. The result is the correctly-named member (never a wrong member/leak/crash),
  // so this is the intended tolerant behavior — NOT a defect.
