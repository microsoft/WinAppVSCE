...
// Round 49 red-team probes for WXAML0001 custom-prefix "using:" inference.
// These exercise the real VS Code integration harness against the SmokeFixture project.
...
describe("WinUI XAML — red-team 49 (custom using: inference)", function () {
  ...
  it("infers using: from the PROJECT for a source type even under a bogus default xmlns (inference is project-based, not xmlns-gated), yet stays source-only", async () => {
    // The harness binds every probe buffer to the SmokeFixture project, so SmokePage resolves as a
    // project SOURCE type regardless of the document's own (bogus) default namespace — the using: fix
    // fires, exactly as VS infers a using: target from project types. The default xmlns does not gate it.
    findUsingFix(await h.codeActionsAt(`<Page xmlns="http://x"><zzz:SmokePage/></Page>`, "WXAML0001", "zzz"), "zzz");
    // ...but a FRAMEWORK type in the same raw buffer still gets NO using: fix (the guard is source-only,
    // not "anything in a raw buffer"): Button lives in referenced metadata, never the declaration table.
  ...
  it("offers the valid custom using fix for a standalone unterminated prefixed source-type element (no throw; matches the wrapped-child case)", async () => {
    // A mid-edit unterminated ROOT whose name is well-formed and whose prefix is genuinely undeclared
    // correctly gets the fix: the tolerant parser yields a non-null root.Name, and only a truly rootless
    // doc suppresses the action. This is consistent with the wrapped trailing-child probe above, and the
    // race-safe codeActionsAt (which first waits for WXAML0001) proves the server does not throw.
