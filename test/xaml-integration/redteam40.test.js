...
// Round 40 red-team probes for WinUI XAML linked editing ranges.
// These focus on false links, precise tag-name boundaries, malformed markup,
// qualified/property-element names, and same-name nesting ambiguity.
...
function assertRegexPattern(wordPattern, expectedText, label) {
  ...
  // VS Code may hand the helper an internal RegExp-like object; helper.js normalizes with String(),
  // which can become "[object Object]". Presence is the important LSP contract here.
...
describe("WinUI XAML red-team 40 — linked editing ranges", function () {
  ...
  // TRIAGE (round 40): the red-team first expected NO link when the caret sits at the EXCLUSIVE END of a
  // tag name (right after the last name character — on the '>' or the space before attributes). That
  // expectation is wrong. It is exactly where a user types to EXTEND the name, so linked editing must
  // stay active or the single most common rename gesture (click at end of tag, type) would silently fail
  // to mirror the close tag. VS Code's own HTML linked-editing reference implementation
  // (microsoft/vscode-html-languageservice, findLinkedEditingRanges) uses INCLUSIVE upper bounds
  //   node.start + '<'.length <= offset && offset <= node.start + '<'.length + tagLength
  // i.e. it links at these exact offsets too. Our TextSpan.ContainsInclusive yields the identical
  // [Start, End] window, so we match the real editor. The red-team's OWN "opening angle bracket" and
  // start-of-name probes above accept the inclusive START boundary, so demanding an exclusive END would
  // be internally inconsistent. These are pinned as MUST-LINK regressions, not defects. The boundary is
  // still tight: the FIRST offset past the name (the '<' of the close tag, or past the element) must not
  // link — the two expectNoLink cases below guard against a greedy over-link.
