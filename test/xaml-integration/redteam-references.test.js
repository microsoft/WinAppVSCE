...
// Find All References target resolution, ranges, deduplication, and malformed input.
...
describe("WinUI XAML red-team 34 — references (Shift+F12)", function () {
  ...
  it("resolves resource-key references across project files (App.xaml declaration + other pages)", async () => {
    // Resource keys are project-wide. A usage of SmokeAccentBrush (declared in App.xaml, used in SmokePage + DiPage) resolves references across every project XAML file (read-only), not just the open document — and must never surface a bin/obj build-output copy.
    const refs = await h.referencesAt(page([
      "<StackPanel>",
