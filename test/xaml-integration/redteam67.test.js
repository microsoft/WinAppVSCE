...
// Round 67 adversarial probes for eager CompletionItem.documentation summaries.
...
describe("WinUI XAML — red-team 67 (completion documentation)", function () {
  ...
  // Wired symbol-bearing builders that the smoke cases and the probes above don't otherwise reach —
  // guards each remaining CompletionDoc(member.Symbol/type) call site against a silent mis-wire.
  ...
  it("red-team 67 Binding markup-argument NAME carries the Binding-property summary", async () => {
    // The server offers named-argument completion only after a positional/comma; the curated x:Bind
    // arg names remain intentionally undocumented (they are hard-coded strings, not symbols).
  ...
  it("red-team 67 element XAML intrinsic x:String carries the System.String summary", async () => {
    // Object-typed content (Button.Content) keeps intrinsics offered as elements; a panel child position is
    // UIElement-typed and correctly excludes them (context-aware element-type narrowing).
