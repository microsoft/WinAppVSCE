...
describe("WinUI XAML — red-team 81 VSM target-property member navigation", function () {
  ...
  // Bare TargetProperty values resolve only through a valid sibling TargetName.
  it("requires a resolvable Storyboard.TargetName for bare Storyboard.TargetProperty member resolution", async () => {
    // A real sibling target resolves the member.
    ...
    // A sibling that names an element that does not exist is equally unresolvable -> also silent (the leak fired here too, since the page fallback ignores the target entirely).
  ...
  it("defers dotted paths but resolves parenthesized Storyboard.TargetProperty groups (round 83)", async () => {
    ...
    // Bare dotted paths remain deferred; explicit (Owner.Property) groups resolve.
    ...
    // Parenthesized groups resolve on their explicit owner type.
