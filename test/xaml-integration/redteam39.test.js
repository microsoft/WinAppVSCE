...
// Round 39 red-team probes for WinUI XAML selectionRange.
// These intentionally stress strict nesting, caret containment, document-boundary
// handling, malformed markup, multi-position requests, and provider determinism.
...
function assertWellFormed(name, probe, caret, ranges) {
  ...
  // VS Code's executeSelectionRangeProvider MERGES our provider's chain with its own built-in
  // (word/bracket/indent) selection-range providers, and that merge can emit two EQUAL consecutive
  // ranges (e.g. a caret in trailing whitespace after the root: the built-in provider contributes an
  // equal pair before our element/document levels). That is a benign editor artifact, not a provider
  // bug — our provider's own chain is strictly nested (pinned by XamlSelectionRangeTests). So collapse
  // consecutive-equal ranges, then require STRICT growth on the deduped chain: this still catches real
  // inversions, partial overlaps, and non-growth while tolerating the built-in merge duplicates.
  const deduped = [];
