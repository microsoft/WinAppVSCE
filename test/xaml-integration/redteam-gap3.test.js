...
describe("WinUI XAML — generate event handler #3 red-team", function () {
  ...
  // x:Class resolution is DISK-based across the whole language service — the project resolver reads the
  // on-disk x:Class (XamlProjectResolver.TryReadClassName -> File.ReadAllText), the same way hover, F12,
  // completion and diagnostics do. A synthetic in-memory buffer therefore CANNOT simulate a missing or
  // renamed class: the resolver still reads the real SmokePage.xaml on disk (x:Class="SmokeFixture.SmokePage").
  // (A wrong/absent x:Class ON DISK correctly yields a null class symbol -> no action, but that is not
  // expressible through the harness's synthetic buffers.) The invariant we CAN and must guarantee under such
  // buffer/disk divergence is corruption-safety: if an action is offered at all, its edit must land in the
  // real, non-generated user partial (SmokePage.xaml.cs) — never a generated .g.cs/.g.i.cs or an obj/bin copy,
  // never a wrong path. That is the property that actually protects the user from a bad cross-file mutation.
  it("keeps any offered code-behind edit on the real user partial when the buffer x:Class diverges from disk", async () => {
    ...
    for (const [name, buffer] of probes) {
      const actions = await h.codeActionsAtCaret(buffer);
      const gen = generate(actions);
      // Under disk-based resolution an action is expected (disk x:Class still resolves); whether or not it
      // appears, it must never be corruption-unsafe.
      if (gen) {
