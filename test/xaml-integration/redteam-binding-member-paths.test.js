...
// Classic {Binding} member-path completion. Probe text avoids asserted names so VS Code word suggestions cannot mask server results.
...
describe("WinUI XAML — red-team 51 (classic {Binding} member paths)", function () {
  ...
  it("does not mistake a BARE POSITIONAL path equal to a redirector keyword for an argument", async () => {
    // A redirector is only ever a named "Source="/"ElementName=" argument; a bare positional first argument is always the Path. So {Binding Source|} inside an Image template must still complete Image.Source, and must not be declined as though "Source" were the Source= redirector argument.
    const buffer = page([
      "<ListView>",
      "  <ListView.ItemTemplate>",
      '    <DataTemplate x:DataType="Image">',
