...
// Round 58 red-team probes for XAML intrinsic aliases as element names (<x:String>, <x:Int32>, ...).
// Assertions discriminate on server-only fields (detail/newText/kind/range) so VS Code word suggestions
// cannot satisfy positives or poison negatives.
...
describe("WinUI XAML — red-team 58 intrinsic element completion", function () {
  ...
  it("keeps intrinsics for a direct object-element (object-typed content) child position", async () => {
    // A ContentControl's Content is object-typed, so intrinsics stay offered as element children. A panel
    // child position (Grid/StackPanel) is UIElement-typed and correctly EXCLUDES the object-derived
    // intrinsics — the context-aware element-type narrowing (see features.test.js "context-aware element
    // types (#1)"), matching Visual Studio.
