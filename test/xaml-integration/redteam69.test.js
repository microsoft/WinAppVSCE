...
describe("WinUI XAML — red-team 69 (x:Bind argument-name Detail parity)", function () {
  ...
  // Promoted from a first-hand missed-defect hunt: the red-team only asserted the Detail SHAPE and
  // parity-with-Binding (which cannot diverge — same symbol). A hardcoded oracle independently pins the
  // exact strings so a silent DescribeMember format change is caught.
  ...
  // Promoted: round 69 touched only the arg-NAME branch. Guard that the round-65 enum arg-VALUE completion
  // (Mode=| -> BindingMode members) is NOT perturbed — its Detail is the enum type, never 'property : ...'
  // or the BindBack curated 'method'.
