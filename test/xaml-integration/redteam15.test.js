...
// Round 15 red-team probes: dense false-positive sweeps for property-element
// diagnostics and x:Bind boolean negation, plus fresh malformed-boundary surfaces.
...
describe("WinUI XAML red-team 15 — fresh malformed and namespace micro-surfaces", function () {
  ...
  it("hover on a property-element name renders the member signature", async () => {
    // Round 18: <Owner.Member> property elements now resolve to the Member property for hover (framework
    // metadata included), so this returns the property signature rather than an empty string.
    const buffer = page([
      "<Grid>",
  ...
  it("hover on a property-element name reports the property's declared type", async () => {
    // Round 18: a second owner/member pair, asserting the rendered signature carries the member name and
    // its collection type (Grid.ColumnDefinitions : ColumnDefinitionCollection).
    const buffer = page([
      "<Grid>",
  ...
  it.skip("navigates from a property-element member name to the property symbol", async () => {
    // KNOWN GAP (blocked on metadata-as-source, needs the Host C bridge): property-element member RESOLUTION
    // is implemented (round 18) and a user-source owner WOULD navigate, but Grid.RowDefinitions is a
    // framework member with no source location, so F12 returns no location here.
    const buffer = page([
      "<Grid>",
      "  <Grid.Row|Definitions>",
      "    <RowDefinition />",
