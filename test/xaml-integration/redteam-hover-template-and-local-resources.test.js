...
// Hover, nested-template scoping, local resources, and binding edge cases.
...
describe("WinUI XAML red-team 8 — nested DataTemplate scoping", function () {
  ...
  it("inner DataTemplate x:DataType shadows the outer template for x:Bind completion", async () => {
    // Hermetic on purpose: no outer x:Bind mentions SmokePage members, so VS Code's word-based suggestions can't surface "GreetingText" from the buffer text — the negative assertion then reflects only the server's scoping (the inner x:String must shadow the outer SmokePage).
    const buffer = page([
      "<ListView>",
