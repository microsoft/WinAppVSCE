...
// Round 13 red-team probes: realistic edge cases around x:Bind path parsing,
// completion context boundaries, diagnostics precision, and outline shape.
...
describe("WinUI XAML red-team 13 — x:Bind negation and path edges", function () {
  ...
  it("resolves parenthesized cast x:Bind paths", async () => {
    // Cast/parenthesized x:Bind expression support (implemented round 26): the member after the
    // cast resolves against the cast target type.
  ...
  it("resolves x:Bind attached-property paths such as (Grid.Row)", async () => {
    // Attached-property path syntax (implemented round 26): hover identifies the attached property
    // on the owner type.
