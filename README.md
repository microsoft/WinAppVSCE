...
| **Code actions** | Press **Ctrl+.** to import and qualify unresolved types, manage namespace declarations, and generate event handlers. If the code-behind has pending edits, save it first when prompted, then retry handler generation so edits use current source positions. |
