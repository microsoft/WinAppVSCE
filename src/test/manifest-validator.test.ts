...
describe('Identity Validation', () => {
    ...
    it('should error when identity name is empty', () => {
        ...
        m.identity.name = '';
        expectError(validateManifest(m, schema), 'identity.name');
    });
    ...
    it('should accept a valid identity name', () => {
        ...
        expectNoError(validateManifest(m, schema), 'identity.name');
    });
    ...
    it('should error when publisher is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'identity.publisher');
    });
    ...
    it('should accept valid publisher CN=Test', () => {
        ...
        expectNoError(validateManifest(m, schema), 'identity.publisher');
    });
    ...
    // The XSD ST_Publisher_2010_v2 pattern does not support backslash escape
    // sequences in unquoted values — these are RFC 2253 features that Windows
    // packaging does not accept.
    it('should reject publisher with escaped comma (not in XSD pattern)', () => {
        const m = makeValidManifest();
        m.identity.publisher = 'CN=Docs\\, Inc';
        expectError(validateManifest(m, schema), 'identity.publisher');
    });

    it('should reject publisher with escaped plus sign (not in XSD pattern)', () => {
        const m = makeValidManifest();
        m.identity.publisher = 'CN=A\\+B';
        expectError(validateManifest(m, schema), 'identity.publisher');
    });

    it('should accept publisher with escaped backslash (XSD pattern allows backslash)', () => {
        const m = makeValidManifest();
        m.identity.publisher = 'CN=Path\\\\Dir';
        expectNoError(validateManifest(m, schema), 'identity.publisher');
    });

    it('should reject publisher with escaped double quote (not in XSD pattern)', () => {
        const m = makeValidManifest();
    ...
    it('should reject publisher with escaped angle brackets (not in XSD pattern)', () => {
        const m = makeValidManifest();
        m.identity.publisher = 'CN=\\<Test\\>';
        expectError(validateManifest(m, schema), 'identity.publisher');
    });

    it('should reject publisher with escaped semicolon (not in XSD pattern)', () => {
        const m = makeValidManifest();
        m.identity.publisher = 'CN=A\\;B';
        expectError(validateManifest(m, schema), 'identity.publisher');
    });

    it('should reject publisher with escaped equals sign (not in XSD pattern)', () => {
        const m = makeValidManifest();
        m.identity.publisher = 'CN=A\\=B';
        expectError(validateManifest(m, schema), 'identity.publisher');
    });
    ...
    it('should accept publisher with trailing backslash (XSD pattern allows backslash)', () => {
        const m = makeValidManifest();
    ...
    it('should accept publisher value starting with space (XSD pattern allows space)', () => {
        const m = makeValidManifest();
    ...
    it('should accept publisher value with backslash-space (XSD pattern allows both)', () => {
        const m = makeValidManifest();
        m.identity.publisher = 'CN=\\ LeadingSpace';
        expectNoError(validateManifest(m, schema), 'identity.publisher');
    });

    it('should reject publisher value with escaped leading # (not in XSD pattern)', () => {
        const m = makeValidManifest();
        m.identity.publisher = 'CN=\\#HashStart';
        expectError(validateManifest(m, schema), 'identity.publisher');
    });

    it('should reject publisher value with escaped trailing space (not in XSD pattern)', () => {
        const m = makeValidManifest();
        m.identity.publisher = 'CN=TrailingSpace\\ ';
        expectError(validateManifest(m, schema), 'identity.publisher');
    });

    it('should reject publisher value with unescaped # in the middle (per XSD schema)', () => {
        const m = makeValidManifest();
        m.identity.publisher = 'CN=Test#Middle';
        expectError(validateManifest(m, schema), 'identity.publisher');
    });

    it('should accept second RDN value starting with space (XSD pattern allows space)', () => {
        const m = makeValidManifest();
    ...
    it('should accept publisher value with forward slash (XSD pattern allows it)', () => {
        const m = makeValidManifest();
        m.identity.publisher = 'CN=Bad/Slash';
        expectNoError(validateManifest(m, schema), 'identity.publisher');
    });

    it('should accept publisher value with backslash-slash (XSD pattern allows it)', () => {
        const m = makeValidManifest();
    ...
    it('should error when version is empty', () => {
        const m = makeValidManifest();
        m.identity.version = '';
        expectError(validateManifest(m, schema), 'identity.version');
    });
    ...
    it('should accept a valid version', () => {
        ...
        expectNoError(validateManifest(m, schema), 'identity.version');
    });
    ...
    it('should not error when resourceId is empty (optional)', () => {
        ...
        expectNoError(validateManifest(m, schema), 'identity.resourceId');
    });

    it('should error when resourceId has invalid chars', () => {
        const m = makeValidManifest();
        m.identity.resourceId = 'bad chars!';
        expectError(validateManifest(m, schema), 'identity.resourceId');
    });
...
describe('PhoneIdentity Validation', () => {
    it('should accept valid GUID for phoneProductId', () => {
        ...
        expectNoError(validateManifest(m, schema), 'phoneIdentity.phoneProductId');
    });
    ...
    it('should error on invalid GUID for phoneProductId', () => {
        ...
        expectError(validateManifest(m, schema), 'phoneIdentity.phoneProductId');
    });
    ...
    it('should accept valid GUID for phonePublisherId', () => {
        ...
        expectNoError(validateManifest(m, schema), 'phoneIdentity.phonePublisherId');
    });
    ...
    it('should error on invalid GUID for phonePublisherId', () => {
        ...
        expectError(validateManifest(m, schema), 'phoneIdentity.phonePublisherId');
    });
...
describe('Properties Validation', () => {
    it('should error when displayName is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'properties.displayName');
    });
    ...
    it('should accept displayName at exactly 256 chars', () => {
        ...
        expectNoError(validateManifest(m, schema), 'properties.displayName');
    });

    it('should error when publisherDisplayName is empty', () => {
        const m = makeValidManifest();
        m.properties.publisherDisplayName = '';
        expectError(validateManifest(m, schema), 'properties.publisherDisplayName');
    });
    ...
    it('should accept publisherDisplayName at exactly 256 chars', () => {
        ...
        expectNoError(validateManifest(m, schema), 'properties.publisherDisplayName');
    });
    ...
    it('should not error when description is empty (optional)', () => {
        ...
        expectNoError(validateManifest(m, schema), 'properties.description');
    });

    it('should error when description exceeds 2048 chars', () => {
        const m = makeValidManifest();
        m.properties.description = 'A'.repeat(2049);
        expectError(validateManifest(m, schema), 'properties.description');
    });
...
describe('Dependencies - Target Device Families', () => {
    it('should error when minVersion is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'dependencies.targetDeviceFamily.0.minVersion');
    });
    ...
    it('should error when maxVersionTested is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'dependencies.targetDeviceFamily.0.maxVersionTested');
    });
    ...
    it('should accept when maxVersionTested equals minVersion', () => {
        ...
        m.dependencies.targetDeviceFamilies[0].maxVersionTested = '10.0.17763.0';
        expectNoError(validateManifest(m, schema), 'dependencies.targetDeviceFamily.0.maxVersionTested');
    });
    ...
    it('should validate multiple target device families independently', () => {
        ...
        expectError(validateManifest(m, schema), 'dependencies.targetDeviceFamily.1.minVersion');
        expectNoError(validateManifest(m, schema), 'dependencies.targetDeviceFamily.0.minVersion');
    });
});
...
describe('Dependencies - Package Dependencies', () => {
    ...
    it('should error when name is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'dependencies.packageDependency.0.name');
    });
    ...
    it('should error when minVersion is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'dependencies.packageDependency.0.minVersion');
    });
    ...
    it('should error when publisher is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'dependencies.packageDependency.0.publisher');
    });
...
describe('Dependencies - Main Package Dependencies', () => {
    it('should error when name is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'dependencies.mainPackageDependency.0.name');
    });
    ...
    it('should accept valid main package dependency name', () => {
        ...
        expectNoError(validateManifest(m, schema), 'dependencies.mainPackageDependency.0.name');
    });
});
...
describe('Dependencies - Driver Constraints', () => {
    ...
    it('should error when constraint name is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'dependencies.driverConstraint.0.name');
    });

    it('should error when constraint minVersion is empty', () => {
        const m = withDriverConstraint(makeValidManifest(), { minVersion: '' });
        expectError(validateManifest(m, schema), 'dependencies.driverConstraint.0.minVersion');
    });
    ...
    it('should error when constraint minDate is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'dependencies.driverConstraint.0.minDate');
    });
...
describe('Dependencies - OS Package Dependencies', () => {
    ...
    it('should error when name is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'dependencies.osPackageDependency.0.name');
    });
    ...
    it('should error when version is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'dependencies.osPackageDependency.0.version');
    });
...
describe('Dependencies - Host Runtime Dependencies', () => {
    ...
    it('should error when name is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'dependencies.hostRuntimeDependency.0.name');
    });

    it('should error when publisher is empty', () => {
        const m = withHostDep(makeValidManifest(), { publisher: '' });
        expectError(validateManifest(m, schema), 'dependencies.hostRuntimeDependency.0.publisher');
    });
    ...
    it('should error when minVersion is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'dependencies.hostRuntimeDependency.0.minVersion');
    });
...
describe('Dependencies - External Dependencies', () => {
    ...
    it('should error when name is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'dependencies.externalDependency.0.name');
    });

    it('should error when publisher is empty', () => {
        const m = withExtDep(makeValidManifest(), { publisher: '' });
        expectError(validateManifest(m, schema), 'dependencies.externalDependency.0.publisher');
    });
    ...
    it('should error when minVersion is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'dependencies.externalDependency.0.minVersion');
    });
...
describe('Resources Validation', () => {
    it('should not error when language is empty', () => {
        ...
        expectNoError(validateManifest(m, schema), 'resources.0.language');
    });

    it('should error when language is numeric only', () => {
        const m = makeValidManifest();
        m.resources[0].language = '123';
        expectError(validateManifest(m, schema), 'resources.0.language');
    });
    ...
    it('should validate multiple resources independently', () => {
        ...
        expectNoError(validateManifest(m, schema), 'resources.0.language');
        expectError(validateManifest(m, schema), 'resources.1.language');
    });
    ...
    it('should error when resource package resource defines more than one attribute type', () => {
        ...
        expectError(validateManifest(m, schema), 'resources.0.language');
        expectError(validateManifest(m, schema), 'resources.0.scale');
    });
    ...
    it('should allow resource package resource with only language', () => {
        ...
        expectNoError(validateManifest(m, schema), 'resources.0.language');
        expectNoError(validateManifest(m, schema), 'resources.0.scale');
    });
...
describe('Applications Validation', () => {
    ...
    it('should error when application id is empty', () => {
        const m = makeValidManifest();
        m.applications[0].id = '';
        expectError(validateManifest(m, schema), 'applications.0.id');
    });
    ...
    it('should accept valid multi-segment application id', () => {
        ...
        expectNoError(validateManifest(m, schema), 'applications.0.id');
    });
    ...
    it('should accept valid entryPoint', () => {
        ...
        expectNoError(validateManifest(m, schema), 'applications.0.entryPoint');
    });
    ...
    it('should error when visual displayName is empty', () => {
        ...
        expectError(validateManifest(m, schema), 'applications.0.visualElements.displayName');
    });
    ...
    it('should accept visual displayName at 256 chars', () => {
        ...
        expectNoError(validateManifest(m, schema), 'applications.0.visualElements.displayName');
    });
    ...
    it('should not error when visual description is empty (optional)', () => {
        ...
        expectNoError(validateManifest(m, schema), 'applications.0.visualElements.description');
    });

    it('should error when visual description exceeds 2048 chars', () => {
        const m = makeValidManifest();
        m.applications[0].visualElements.description = 'A'.repeat(2049);
        expectError(validateManifest(m, schema), 'applications.0.visualElements.description');
    });
    ...
    it('should error when backgroundColor is invalid hex', () => {
        ...
        expectError(validateManifest(m, schema), 'applications.0.visualElements.backgroundColor');
    });
    ...
    it('should accept hex color #FFFFFF', () => {
        ...
        expectNoError(validateManifest(m, schema), 'applications.0.visualElements.backgroundColor');
    });
    ...
    it('should error when square150x150Logo is empty string', () => {
        ...
        expectError(validateManifest(m, schema), 'applications.0.visualElements.square150x150Logo');
    });
    ...
    it('should accept valid square150x150Logo with .png', () => {
        ...
        expectNoError(validateManifest(m, schema), 'applications.0.visualElements.square150x150Logo');
    });

    it('should error when square44x44Logo is empty string', () => {
        const m = makeValidManifest();
        m.applications[0].visualElements.square44x44Logo = '';
        expectError(validateManifest(m, schema), 'applications.0.visualElements.square44x44Logo');
    });
    ...
    it('should not error when wide310x150Logo is null (optional)', () => {
        ...
        expectNoError(validateManifest(m, schema), 'applications.0.visualElements.wide310x150Logo');
    });

    it('should error when wide310x150Logo is empty string', () => {
        const m = makeValidManifest();
        m.applications[0].visualElements.wide310x150Logo = '';
        expectError(validateManifest(m, schema), 'applications.0.visualElements.wide310x150Logo');
    });
    ...
    it('should not error when square71x71Logo is null', () => {
        ...
        expectNoError(validateManifest(m, schema), 'applications.0.visualElements.square71x71Logo');
    });

    it('should error when square71x71Logo is empty string', () => {
        const m = makeValidManifest();
        m.applications[0].visualElements.square71x71Logo = '';
        expectError(validateManifest(m, schema), 'applications.0.visualElements.square71x71Logo');
    });

    it('should not error when square310x310Logo is null', () => {
        const m = makeValidManifest();
        m.applications[0].visualElements.square310x310Logo = null;
        expectNoError(validateManifest(m, schema), 'applications.0.visualElements.square310x310Logo');
    });
    ...
    it('should error when square310x310Logo has .svg extension', () => {
        ...
        expectError(validateManifest(m, schema), 'applications.0.visualElements.square310x310Logo');
    });

    it('should not error when badgeLogo is null', () => {
        const m = makeValidManifest();
        m.applications[0].visualElements.badgeLogo = null;
        expectNoError(validateManifest(m, schema), 'applications.0.visualElements.badgeLogo');
    });

    it('should error when badgeLogo is empty string', () => {
        const m = makeValidManifest();
        m.applications[0].visualElements.badgeLogo = '';
        expectError(validateManifest(m, schema), 'applications.0.visualElements.badgeLogo');
    });

    it('should not error when splashScreenImage is null', () => {
        const m = makeValidManifest();
        m.applications[0].visualElements.splashScreenImage = null;
        expectNoError(validateManifest(m, schema), 'applications.0.visualElements.splashScreenImage');
    });
    ...
    it('should error when splashScreenImage has .tiff extension', () => {
        ...
        expectError(validateManifest(m, schema), 'applications.0.visualElements.splashScreenImage');
    });
